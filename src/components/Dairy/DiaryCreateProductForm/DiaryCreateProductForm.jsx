...
const DiaryCreateProductForm = () => {
  ...
  return (
    <>
      ...
      <h3 className={s.title}>Додайте новий продукт до бази</h3>
      <form className={s.form} onSubmit={handleSubmit(onSubmit)}>
        <div className={s.overlayLabel}>
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <TextFieldDefault
                value={value}
                placeholder={'Назва продукту (Лічі)'}
                name={'query'}
                control={control}
        ...
        {isWarning && isMobile && <p className={s.text_message}>{isWarning}</p>}
      ...
      {isWarning && isTabletDesktop && <p className={s.text_message}>{isWarning}</p>}
      </div>
    </>
  );
};
