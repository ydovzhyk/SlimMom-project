...
const DiaryProductsListItem = () => {
  ...
  if (userData && Object.keys(userData).length > 0) {
    ...
    const userDataFromLocalStorage = JSON.parse(localStorage.getItem(`user_${userId}_userData`));
  ...
  return (
    <>
      {eatenProductsList &&
        eatenProductsList.map(({ id, title, weight, kcal, groupBloodNotAllowed}) => {
          return (
            <li key={id} className={s.item}>
              {checkBloodType(groupBloodNotAllowed) && <FaTint className={s.myBloodIconTrue} title="Продукт підходить вашій групі крові" />}
              {!checkBloodType(groupBloodNotAllowed) && <FaTint className={ s.myBloodIconFalse} title="Продукт не підходить вашій групі крові" />}
